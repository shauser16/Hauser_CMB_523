...
```{r}
n= 104000965#pick a large number for N, around one-hundred million or so
res = runif(n) + 1
```

QUESTION 1: What does the above code do?

ANSWER 1: The above code allows us to create results that are easier to produce over time. Over time, the created seed from this code will create order to the "random" numbers that are generated.
...
```{r}
#write code that runs this code 3 times using a loop, saves the amount of time it took to run each time, then prints the average (cis.time)
num_runs <-3
cis_time <-numeric(num_runs)
...
cis_time[run] <- as.numeric(end_time - start_time) #store the calculated runs times
cat("Run", run, "completed in", cis_time[run], "seconds")
#print the average of the three runs 
average_runtime <- mean(cis_time)
...
ANSWER 2: the "i" value in the above code represents the variable which encodes for logging the times between each run. The value it takes between each run is the value of each iteration. For instance, the value of "i" could be 33 if the contents of "res" is adequate.
...
<YOUR ANSWER HERE> "i" and "j" represent the different loop variables. since the variable "i" is formed with nrow, it is used to utilize the data from each row of the system. Likewise, because variable "j" is encoded with ncol, the variable "j" checks for lowercase "a" in each column.
...
<YOUR ANSWER HERE> Since the data for nrow and ncol is being stored and it is the only change made, this means on the previous method the nrow and ncol functions were being called each time a run was performed. Because of this, r had to generate a new set of nrow and ncol along with the other data, making run times longer. 
...
<YOUR ANSWER HERE> This loop executes 3 times.
...
<WRITE YOUR ANSWER HERE> Method 2 was the fastest and method 4 was the slowest, the difference being nearly 8 seconds between the two methods. Method 4 ran at .758x the speed that method 2 ran.
...
```{r}
#Your code here/ reference lines 109-112
#Count the total number of 'T's'
T_count_total<- sum(sequencing_matrix == "T", na.rm = TRUE)

cat("total number of 'T's' in the sequencing matrix", T_count_total)
```
